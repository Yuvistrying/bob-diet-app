...
async function findReferenceIssue() {
  ...
  try {
    ...
    console.log(
      `Found ${issues.nodes.length} issues with "Implementation Reference Videos" in title\n`,
    );
    ...
    if (issues.nodes.length > 0) {
      for (const issue of issues.nodes) {
        ...
        console.log(
          `URL: https://linear.app/${team.key}/issue/${issue.identifier}`,
        );
        console.log("---");
      }
    } else {
      ...
      for (const issue of recentIssues.nodes) {
        ...
        console.log(
          `- "${issue.title}" (${issue.identifier}) - ${new Date(issue.createdAt).toLocaleString()}`,
        );
      }
    }
  } catch (error) {
