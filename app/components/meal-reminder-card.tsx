    ...
    <Card className="bg-amber-50 border-amber-200 mb-4">
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div className="bg-amber-100 p-2 rounded-lg">{getMealIcon()}</div>
          <div className="flex-1 space-y-2">
            ...
            <p className="text-sm text-amber-700">
              {mealType === "breakfast" &&
                "Start your day right by tracking your morning meal."}
              {mealType === "lunch" && "Don't forget to log your midday fuel!"}
              {mealType === "dinner" &&
                "End your day strong by logging your evening meal."}
              {mealType === "snack" && "Had a snack? Every bite counts!"}
            </p>
            <p className="text-xs text-amber-600">Typical time: {timeWindow}</p>
...
export function MissedMealsSummary({
  missedMeals,
  onAddMeals,
}: MissedMealsSummaryProps) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 space-y-2">
            <h3 className="font-medium text-orange-900">
              You missed logging {missedMeals.length} meal
              {missedMeals.length > 1 ? "s" : ""} today
            </h3>
            <p className="text-sm text-orange-700">
              {missedMeals.join(", ")} - Would you like to add them now? It's
              not too late!
            </p>
            <Button
              size="sm"
