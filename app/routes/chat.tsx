...
export default function Chat() {
        ...
        try {
          ...
          if (parsed.date === today) {
            ...
            if (parsed.confirmed && parsed.confirmed.length > 0) {
              console.log(
                "Loading confirmed bubbles from localStorage:",
                parsed.confirmed,
              );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (currentConfirmations.length > 0 || confirmedFoodLogs.size > 0) {
        ...
        console.log(
          "Saving confirmed bubbles to localStorage:",
          Array.from(confirmedFoodLogs),
        );
  ...
  useEffect(() => {
    if (chatHistory && !hasLoadedHistory) {
      ...
      if (persistedConfirmations.length > 0) {
        ...
        const sortedConfirmations = [...persistedConfirmations].sort(
          (a, b) => b.index - a.index,
        );

        sortedConfirmations.forEach((conf) => {
          // Find if this confirmation already exists in the history
          const existingIndex = formattedHistory.findIndex(
            (msg, idx) =>
          ...
          if (existingIndex === -1) {
            ...
            if (
              insertIndex >= 0 &&
              insertIndex + 1 <= formattedHistory.length
            ) {
      ...
      setTimeout(() => {
        ...
        if (savedConfirmations) {
          try {
            ...
            if (parsed.confirmed && parsed.confirmed.length > 0) {
              console.log(
                "Restoring confirmed state after chat history loaded:",
                parsed.confirmed,
              );
              setConfirmedFoodLogs(new Set(parsed.confirmed));
            }
  ...
  const detectOnboardingStep = () => {
    ...
    if (
      content.includes("biological sex") ||
      content.includes("male or female")
    ) {
      return "gender";
    } else if (
      content.includes("activity level") ||
      content.includes("how active")
    ) {
      return "activity_level";
    } else if (
      content.includes("would you like to") &&
      content.includes("describe") &&
      (content.includes("goal") || content.includes("situation"))
    ) {
      ...
      content.includes("stealth mode")
    ) {
      return "display_mode";
    } else if (
      content.includes("current weight") &&
      !content.includes("goal weight")
    ) {
      return "current_weight";
    } else if (
      content.includes("goal weight") ||
      content.includes("target weight")
    ) {
    ...
    >
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={async () => {
                ...
                if (
                  confirm("Start a new chat? Your food logs will be saved.")
                ) {
                  setIsLoading(true);
                  try {
                    // Start new session
        ...
        {!isOnboarding && (
          <div className="p-2 border-b border-gray-200 dark:border-gray-800 space-y-1.5">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-2 shadow-sm">
              <div className="text-xs text-gray-500 dark:text-gray-400 font-mono text-center mb-1">
                🔥 Nutrition
              </div>
      ...
      <div className="flex-1 relative overflow-hidden bg-gray-50 dark:bg-gray-950">
        <div
          ...
          style={{ minHeight: 0, paddingBottom: "140px" }}
        >
          <ClientOnly>
                        ...
                        {isConfirmed ? (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-green-700 dark:text-green-300">
                                ...
                                {editedFoodItems[confirmId]
                                  ? editedFoodItems[confirmId].reduce(
                                      (sum: number, item: any) =>
                                        sum + (item.calories || 0),
                                      0,
                                    )
                              ...
                              <span className="text-sm text-green-600 dark:text-green-400">
                                •{" "}
                                {editedFoodItems[confirmId]?.length ||
                                  args.items.length}{" "}
                                items
                              </span>
                            </div>
                          ...
                          <div className="max-w-[80%] px-4 py-3 rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm">
                            ...
                            <div className="space-y-1">
                              {(editedFoodItems[confirmId] || args.items).map(
                                (item: any, i: number) => (
                                  <div
                                    key={i}
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                              ...
                              {!isStealthMode && (
                                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1 font-mono">
                                  {args.totalProtein}g protein •{" "}
                                  {args.totalCarbs}g carbs • {args.totalFat}g
                                  fat
                                </div>
                              )}
                            </div>
                            <div className="mt-4 space-y-3">
                                ...
                                <Button
                                  ...
                                  onClick={async () => {
                                    ...
                                    if (editedFoodItems[confirmId]) {
                                      ...
                                      const confirmCall = updatedMessage[
                                        index
                                      ].toolCalls?.find(
                                        (tc) => tc.toolName === "confirmFood",
                                      );
                                      if (confirmCall) {
                                        ...
                                        confirmCall.args = {
                                          ...confirmCall.args,
                                          items: editedItems,
                                          totalCalories: editedItems.reduce(
                                            (sum: number, item: any) =>
                                              sum + (item.calories || 0),
                                            0,
                                          ),
                              ...
                              <p className="text-xs text-muted-foreground dark:text-gray-400 text-center">
                                If this isn't accurate, just tell me what to
                                change in the chat below
                              </p>
                            </div>
                          </div>
                        )}
                ...
                >
                  {message.role === "user" ? (
                    <>
                      ...
                      {message.content &&
                        message.content !== "[Photo uploaded]" && (
                          <div
                            className={cn(
      ...
      {isOnboarding && currentOnboardingStep && (
        <div
          className="fixed bottom-0 left-0 right-0 bg-gray-50/90 dark:bg-gray-900/90 backdrop-blur-sm px-4 py-3 shadow-[0_-2px_10px_rgba(0,0,0,0.1)] dark:shadow-[0_-2px_10px_rgba(255,255,255,0.05)]"
      ...
      <div
        className="fixed bottom-0 left-0 right-0 p-4 bg-gray-50 dark:bg-gray-950"
        style={{ paddingBottom: "76px" }}
      >
        <div className="mx-auto w-full">
          <form onSubmit={handleSubmit} className="relative focus:outline-none">
