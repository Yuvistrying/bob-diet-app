...
import { useConvexStreamingChat } from "~/app/hooks/useConvexStreamingChat";
...
export default function ConvexStreamingDemo() {
  ...
  const createThread = useMutation(api.agentActions.createThread);
  const saveAgentThreadId = useMutation(api.userPreferences.saveAgentThreadId);
  
  // Initialize with saved thread ID
  const [initialThreadId, setInitialThreadId] = useState<string | null>(null);
  
  useEffect(() => {
    if (preferences?.agentThreadId) {
      setInitialThreadId(preferences.agentThreadId);
    }
  }, [preferences?.agentThreadId]);
  
  // Use native Convex streaming hook
  const {
    messages,
    isStreaming,
    threadId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!activeThreadId) {
        ...
        await saveAgentThreadId({ threadId: activeThreadId });
      }
      
      // Send message with streaming
      await sendMessage(userInput, activeThreadId);
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="font-semibold text-lg text-gray-900 dark:text-gray-100">
              Bob - Native Convex Streaming 🚀
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Using @convex-dev/agent streaming features
            </p>
          </div>
          {todayStats && profile && (
          ...
          {messages.map((message) => {
            ...
            if (message.toolCalls && message.toolCalls.length > 0) {
              ...
              if (confirmFoodCall) {
                ...
                return (
                  <div key={message.key} className="space-y-4">
                    {/* Assistant message */}
