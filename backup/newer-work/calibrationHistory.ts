    ...
    if (!latestCalibration || latestCalibration.createdAt < twoWeeksAgo) {
      ...
      if (weightLogs.length >= 7 && foodLogs.length >= 20) {
        return {
          needed: true,
          reason:
            "It's been over 2 weeks since your last calibration, and you have enough data for an accurate analysis.",
          dataPoints: {
    ...
    if (recentWeights.length >= 5) {
      ...
      if (weightChange < 0.5 && profile.goal !== "maintain") {
        return {
          needed: true,
          reason:
            "You seem to have hit a plateau. A calibration could help break through it.",
          dataPoints: {
            daysOnPlateau: 10,
