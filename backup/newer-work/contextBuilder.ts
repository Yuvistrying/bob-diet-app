    ...
    switch (contextType) {
      case "meal_planning":
        ...
        specificContext = {
          ...
          mealPatterns: {
            averageMealTimes: Object.entries(mealTimes).reduce((acc: any, [meal, times]: [string, any]) => {
      ...
      case "calibration":
        ...
        const dailyTotals = foodLogs.reduce((acc: any, log) => {
          if (!acc[log.date]) {
            acc[log.date] = { calories: 0, protein: 0, carbs: 0, fat: 0, meals: 0 };
          }
        ...
        const perfectDays = Object.values(dailyTotals).filter((day: any) => 
          profile && Math.abs(day.calories - profile.dailyCalorieTarget) < 100
        ).length;
        ...
        specificContext = {
          weightData: {
            ...
            totalChange: weightHistory.length >= 2 
          ...
          nutritionData: {
            ...
            completeDays: Object.values(dailyTotals).filter((day: any) => day.meals >= 3).length,
            averageDailyCalories: daysWithData > 0
              ? Math.round(
                  Object.values(dailyTotals).reduce((sum: number, day: any) => sum + day.calories, 0) / daysWithData
                )
              : 0,
      ...
      case "progress_check":
        ...
        specificContext = {
          recentProgress: {
            lastWeekWeight: recentWeights.length >= 7 ? recentWeights[6].weight : null,
            currentWeight: recentWeights[0]?.weight,
            weeklyChange: recentWeights.length >= 7 
