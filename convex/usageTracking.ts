...
export const checkUsageLimit = query({
  ...
  handler: async (ctx, args) => {
    ...
    const fieldName = args.usageType === "chat" ? "chatCount" : "photoAnalysisCount";
    const currentCount = usage[fieldName as keyof typeof usage] as number || 0;
    ...
    return {
      today: {
        chats: {
          ...
          remaining: subscription ? null : limits.chat - (usage?.chatCount || 0)
        },
        photos: {
          used: usage?.photoAnalysisCount || 0,
          limit: subscription ? null : limits.photoAnalysis,
          remaining: subscription ? null : limits.photoAnalysis - (usage?.photoAnalysisCount || 0)
        }
      },
      isPro: !!subscription,
      modelUsage: {
