...
export const cleanupTestUser = mutation({
  ...
  handler: async (ctx, { email }) => {
    ...
    const userProfile = profiles.find(p => p.name.toLowerCase() === email.split('@')[0].toLowerCase());
    if (!userProfile) {
      console.log("No user profile found. Try using the username part of the email.");
      return { 
        success: false, 
        message: "No user found. Try using just the username part of the email (before @)",
    ...
    return {
      ...
      deletions,
      clerkNote: "⚠️ IMPORTANT: You must also delete this user from the Clerk Dashboard at https://dashboard.clerk.com/"
    };
  },
});
