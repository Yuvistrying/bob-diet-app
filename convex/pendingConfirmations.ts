...
export const getLatestPendingConfirmation = query({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(
      `[getLatestPendingConfirmation] Querying for thread: ${args.threadId}`,
    );
    ...
    if (pending) {
      console.log(
        `[getLatestPendingConfirmation] Found pending confirmation:`,
        {
          id: pending._id,
      ...
      console.log(
        `[getLatestPendingConfirmation] No pending confirmation found`,
      );
    }
    ...
    if (pending && pending.expiresAt < Date.now()) {
      console.log(
        `[getLatestPendingConfirmation] Confirmation expired, returning null`,
      );
      return null;
    }
...
export const confirmPendingConfirmation = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(
      `[confirmPendingConfirmation] Marking confirmation as confirmed:`,
      args.confirmationId,
    );
    ...
    if (!confirmation || confirmation.userId !== identity.subject) {
      console.log(
        `[confirmPendingConfirmation] Confirmation not found or wrong user`,
      );
      throw new Error("Confirmation not found");
    }

    console.log(
      `[confirmPendingConfirmation] Current status:`,
      confirmation.status,
    );
    await ctx.db.patch(args.confirmationId, { status: "confirmed" });
    console.log(
      `[confirmPendingConfirmation] Successfully marked as confirmed`,
    );
