...
export const chat = action({
  ...
  handler: async (ctx, args): Promise<{ response: string; action: string | null; showUpgrade?: boolean; data?: any }> => {
    ...
    const bobResponse = await processBobMessage(ctx, args.message, context, args.imageUrl);
...
function isFoodDescription(message: string): boolean {
  const foodKeywords = ['ate', 'had', 'eating', 'breakfast', 'lunch', 'dinner', 'snack', 'meal', 'food', 'drink', 'drank'];
  const lower = message.toLowerCase();
  return foodKeywords.some(keyword => lower.includes(keyword));
}

function getWeightFeedback(current: number, previous: number | undefined, goal: string): string {
...
async function estimateFoodFromDescription(description: string): Promise<FoodEstimate> {
